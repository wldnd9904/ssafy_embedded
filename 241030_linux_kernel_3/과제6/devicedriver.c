...
static volatile uint32_t *GPIO21_CTRL;
static volatile uint32_t *PAD_GPIO21;

#define GPIO20_OFFSET 0xa4
#define GPIO21_OFFSET 0xac

static void ledtoggle(int pinno){
    pr_info("%d toggle\n", pinno);
    switch(pinno){
		case 20:
		    *GPIO20_CTRL ^= (0x1<<12);
...
static ssize_t deviceFile_ioctl(struct file *filp, unsigned int cmd, unsigned long arg){
	int pinno;
	copy_from_user(&pinno, (int *)arg, sizeof(int));
	pr_info("%d toggle\n", pinno);
    switch(cmd){
        case _IO(0,3):
			ledtoggle(pinno);
...
static int __init deviceFile_init(void){
    ...
    GPIO21_CTRL = IOBank0 + GPIO21_OFFSET/4;

    //PAD_GPIO20 offset 0x54
    PAD_GPIO20 = PADBank0 + 0x54/4;
    PAD_GPIO21 = PADBank0 + 0x58/4;
