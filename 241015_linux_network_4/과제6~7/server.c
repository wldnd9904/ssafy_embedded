    ...
    while (1) {
        ...
        if(!strcmp("save",command)) {
            ...
            for (int i = 0; i < MAX_CLIENT_CNT; i++){
                if (client_sock[i] != 0){
					char ret[100];
                    sprintf(ret, "%s KEY: %s, VALUE: %s saved.", name, key, value);
                    write(client_sock[i], ret, strlen(ret));
                }
            }
        }
        ...
        if(!strcmp("read",command)) {
            ...
            char* value = dbRead(key);
            // 모든 클라이언트에게 메시지를 브로드캐스트
            for (int i = 0; i < MAX_CLIENT_CNT; i++){
                if (client_sock[i] != 0){
					char ret[100];
                    sprintf(ret, "%s KEY: %s, VALUE: %s", name, key, value);
                    write(client_sock[i], ret, strlen(ret));
                }
            }
        }
        ...
        if(!strcmp("echo",command)) {
            char* bufremain = buf+strlen(name)+6;
            int offset = 0;
			while(bufremain[offset]){
				if(bufremain[offset]>= 'a' && bufremain[offset] <= 'z') bufremain[offset]-=32;
				offset++;
			}
			// 모든 클라이언트에게 메시지를 브로드캐스트
            for (int i = 0; i < MAX_CLIENT_CNT; i++){
                if (client_sock[i] != 0){
                    char ret[100];
                    sprintf(ret, "%s %s", name, bufremain);
                    write(client_sock[i], ret, strlen(ret));
                }
            }
        }
