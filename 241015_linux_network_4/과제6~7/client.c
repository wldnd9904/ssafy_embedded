...
void* sendMsg() {
    ...
    while (!exitFlag) {
        ...
        if (!strcmp(msg, "close\n")) {  // 'exit' 입력 시 종료 플래그 설정
            closeFlag = 1;
			char* exitMsg = "exit";
            write(client_sock, exitMsg, strlen(exitMsg));  // 서버로 'exit' 메시지 전송
...
int main(int argc, char* argv[]) {
    while (1) {
        ...
        char user[100];
        while (1) {
            printf("input connect [IP] [PORT] [name]\n");
            fgets(input, sizeof(input), stdin);  // 입력받은 문자열을 input에 저장
			if(strcmp("exit",input)==0){
				exitFlag = 1;
				break;
			}
            sscanf(input, "%s %s %s %s", command, IP, PORT, user);
            strcpy(SERVER_IP, IP);  // 서버 IP 주소 저장
            strcpy(SERVER_PORT, PORT);  // 서버 포트 번호 저장
            sprintf(name, "[%s]", user);
            break;
        }
