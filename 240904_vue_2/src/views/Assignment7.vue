...
const appendTodo = () => {
  ...
  const id = todos.value.reduce((prev, curMenu) => Math.max(prev, curMenu.id), 0) + 1;
...
const isAllDone = computed(() => todos.value.reduce((prev, cur) => prev & cur.isDone, true));
</script>

<template>
  <h1 class="fw-bold">과제7 - Todo List</h1>
  <form class="input-group mb-4" onsubmit="return false">
    <input type="text" class="form-control" v-model="title" placeholder="할 일을 입력하세요" />
    <button class="btn btn-outline-secondary" @click="appendTodo">추가</button>
  </form>

  <ul v-if="todos.length" class="list-group mb-4">
    <li v-for="todo in filteredTodos" class="d-flex p-2 gap-2 list-group-item" :key="todo.id">
      <input type="checkbox" :checked="todo.isDone" @change="toggleIsDone(todo.id)" />
